 ...
 */
@Controller
@RequestMapping(value = "commonGm")
public class CommonGmController {

    private Logger logger = LoggerFactory.getLogger(CommonGmController.class);

    @Autowired
    private ServerListService serverListService;
    @Autowired
    CommonGmService commonGmService;

    @RequestMapping(value = "gmQuery")
    public ModelAndView root() {
        ModelAndView view = new ModelAndView();
        serverListService.addGlobalAttribute(view);
        view.setViewName("commonGm/commonGm");
        return view;
    }
    ...
    public GmCommandDto sendMail(HttpSession session,
        ...
        return commonGmService.sendMail(session.getAttribute("username").toString(), IpUtil.remoteIp(request), AsyncKey.GM_NOTE, "gmMail", mailContent, mailReward, server);
    }
    ...
    public GmCommandDto sendNote(HttpSession session,
        ...
        return commonGmService.sendNote(session, IpUtil.remoteIp(request), AsyncKey.GM_NOTE, startTime, times, interval, content, server);
    }
    ...
    public OneMailDto sendOneMail(HttpSession session,
        ...
        try {
            ...
            return commonGmService.sendOneMail(actor, IpUtil.remoteIp(request), platformKey.getPlatformId(), platformKey.getServerId(), "gmMailToOne", v);
        } catch (Exception e) {
            logger.error(e.getMessage(), e);
