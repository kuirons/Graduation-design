...
//        }
     ...
     * @return
     */
    public static Result getRow(String tableName, byte[] row, Filter... filters) {
        Table table = getTable(tableName);
        Result rs = null;
        if (table != null) {
            try {
     ...
     * @param filter       可选的过滤器参数
     * @return
     */
    public static ResultScanner getScan(String tableName, HashMap<String, List<String>> paramHashMap, Filter filter) {
        Table table = getTable(tableName);
        ResultScanner results = null;
        if (table != null) {
            try {
                if (paramHashMap == null) {
                    paramHashMap = new HashMap<String, List<String>>();
                }
